...
function ProductEdit(props) {
  ...
  const initProduct = async() => {
    try {
      ...
      const product = await axios.post('/products/id',param)
      if(product?.data?.product?.length > 0) {
        setNameProduct(product.data.product[0].product_name)
        setPrice(product.data.product[0].base_price)
        setImg(product.data.product[0].image)
        setNoiDung(product.data.product[0].description)
      } else {
        setNameProduct('')
        setPrice('')
        setImg('')
  ...
  const handleSave = async() => {
    try {
      ...
      const sResult = await axios.post('products/update',param)
